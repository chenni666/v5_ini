...
TARGET_SUFFIX = Path("DeltaForce") / "Saved" / "Config" / "WindowsClient" / "Engine.ini"
DEFAULT_ROOTS = [Path(r"C:\\WeGameApps"), Path(r"D:\\WeGameApps")]


def find_engine_configs(root_dirs: Iterable[Path]) -> List[Path]:
    """Search the supplied roots for Engine.ini files under the expected Delta Force path."""
    matches: List[Path] = []
    ...
    for root in root_dirs:
        ...
        for candidate in root.rglob("Engine.ini"):
            if candidate.is_file() and TARGET_SUFFIX.parts == candidate.parts[-len(TARGET_SUFFIX.parts):]:
...
def main() -> None:
    ...
    matches = find_engine_configs(roots)

    if not matches and not args.global_search and roots == DEFAULT_ROOTS:
        print("未找到配置文件，开始进行全盘扫描，这可能需要一些时间……")
        matches = find_engine_configs(list_drive_roots())

    if not matches:
        print("No matching Engine.ini files were found.")
        return
